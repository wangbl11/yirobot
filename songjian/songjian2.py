...
names=["经尿道膀胱","经尿道前列腺","腹腔镜","输尿管镜","经皮肾镜"]
#
...
for one in zg_sheet.values:
    if  len(one[4])>0:
        ...
        for name in names:
            if name in one[4]:
                  two=results.get(name,copy.deepcopy(common.default_one))
...
common.compositeResults(results,header,"2.xlsx")
...
# # 保存
# try:
#     save.to_excel("2.xlsx",index=False)
# except:
#     pass